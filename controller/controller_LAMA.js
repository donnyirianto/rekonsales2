            ...
            for (const item of looper) { 
               ...
               results.forEach( async (r) => { 
                ...
                const querysql = `SELECT  '${r.kdcab}' as kdcab, '${r.kdtk}' as kdtk, '${r.nama}' as nama_toko,
                    ...
                    SUM(IF(RTYPE='J',GROSS - if(sub_bkp='Y',ifnull(PPN,gross*ppn_rate/(1+ppn_rate)),0),0)) AS TJUAL,
                    SUM(IF(RTYPE='D',GROSS - if(sub_bkp='Y',ifnull(PPN,gross*ppn_rate/(1+ppn_rate)),0),0)) AS TRET,
                    SUM(IF(RTYPE='J',QTY,0)) AS JQTY,
                    SUM(IF(RTYPE='D',QTY,0)) AS DQTY,
                    SUM(IF(RTYPE='J',IF(SUB_BKP IN ('P','W','G'),ifnull(PPN,gross*ppn_rate/(1+ppn_rate)),0),IF(SUB_BKP IN ('P','W','G'),ifnull(PPN,gross*ppn_rate/(1+ppn_rate)) *-1,0))) AS BBS_PPN
                    FROM MTRAN
                    WHERE 
